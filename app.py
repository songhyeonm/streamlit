    ...
    if submit:
        if not name: 
            ...
            pokemons.append({
                "name": name,
                "types": types,
                "image_url": image_url if image_url else "./images/default.png" ## image_url이 있으면 넣고 그렇지 않으면 "./images/default.png"를 지정
            })




## 화면이 작아질 때 column에 인덱스 순서대로 배열하기 위한 반복문
    ...
    for j in range(len(row_pokemons)):
        with cols[j]:
            ...
            with st.expander(label=f"**{i+j+1}. {pokemon['name']}**", expanded=True):
                ...
                ## 구분자 "/"를 기준으로 모든 배열의 element들을 하나로 합친다
